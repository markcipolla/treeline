...
func TestAssigneeLabel(t *testing.T) {
	for _, tc := range []struct{ name, displayName, want string }{
		...
		{"mark.cipolla@example.com", "mark.cipolla", "mark.cipolla"},
		{"tristan@example.com", "tristan", "tristan"},
		// no displayName to fall back on: keep the local part
		{"chayut@example.com", "", "chayut"},
		{"", "brendan", "brendan"},
		{"", "", ""},
	} {
...
func TestIssueAssigneeDecoding(t *testing.T) {
	const payload = `[
	  ...
	  {"identifier":"LAB-2","title":"two","assignee":{"name":"mark.cipolla@example.com","displayName":"mark.cipolla"}},
